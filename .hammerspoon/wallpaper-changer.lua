...
local function get_files(folder_path)
  _G.assert((nil ~= folder_path), "Missing argument folder-path on .hammerspoon/wallpaper-changer.fnl:48")
...
local function hassupportedUTI_3f(UTI_list, path)
  _G.assert((nil ~= path), "Missing argument path on .hammerspoon/wallpaper-changer.fnl:58")
  _G.assert((nil ~= UTI_list), "Missing argument UTI-list on .hammerspoon/wallpaper-changer.fnl:58")
...
local function get_supported_file_paths(UTIs, file_paths)
  _G.assert((nil ~= file_paths), "Missing argument file-paths on .hammerspoon/wallpaper-changer.fnl:67")
  _G.assert((nil ~= UTIs), "Missing argument UTIs on .hammerspoon/wallpaper-changer.fnl:67")
  local file_list = {}
...
local function calculate_next_index(file_list, current_idx)
  _G.assert((nil ~= current_idx), "Missing argument current-idx on .hammerspoon/wallpaper-changer.fnl:75")
  _G.assert((nil ~= file_list), "Missing argument file-list on .hammerspoon/wallpaper-changer.fnl:75")
...
local function print_screen_details(screen)
  _G.assert((nil ~= screen), "Missing argument screen on .hammerspoon/wallpaper-changer.fnl:121")
  local _12_
  do
    local frame = screen:frame()
    _12_ = (frame.w .. " x " .. frame.h .. "px")
  end
  return print(("Screen " .. screen:name() .. " - " .. _12_))
end
local function iterate_screens(screen_list, func, _3fargs)
  _G.assert((nil ~= func), "Missing argument func on .hammerspoon/wallpaper-changer.fnl:127")
  _G.assert((nil ~= screen_list), "Missing argument screen-list on .hammerspoon/wallpaper-changer.fnl:127")
...
local function change_wallpaper_on_screen(screen, file_path)
  _G.assert((nil ~= file_path), "Missing argument file-path on .hammerspoon/wallpaper-changer.fnl:133")
  _G.assert((nil ~= screen), "Missing argument screen on .hammerspoon/wallpaper-changer.fnl:133")
  do
...
local function run_rotator(folder)
  _G.assert((nil ~= folder), "Missing argument folder on .hammerspoon/wallpaper-changer.fnl:141")
  ...
  if not (image_paths and (#image_paths > 0) and screens and main_screen) then
    ...
    if not next_index then
      return print("Error: Next index position is nil....")
    else
      local next_image = image_paths[next_index]
      if not next_image then
        return print("Error: next image is nil...")
      else
        hs.settings.set("wallpaper-index", next_index)
        print(("Setting next image to " .. next_image))
        iterate_screens(screens, change_wallpaper_on_screen, image_paths[next_index])
        return print("Changed wallpaper successfully!")
      end
    end
  end
end
...
local function init_WALLPAPER_DUR_MINS(duration)
  _G.assert((nil ~= duration), "Missing argument duration on .hammerspoon/wallpaper-changer.fnl:184")
...
local function resume_wallpaper_timer()
  if M["wallpaper-timer"]:running() then
    ...
    if not (remaining_time and (remaining_time > 0)) then
      ...
      M["wallpaper-timer"]:setNextTrigger(remaining_time)
