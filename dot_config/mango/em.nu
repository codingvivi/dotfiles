...
def "main org roam dailies today goto" [] {
  ...
  emacsclient -c -F '((name . "OrgDailyFrame"))' -e '(let ((display-buffer-overriding-action (quote ((display-buffer-same-window))))) (org-roam-dailies-goto-today))'
}
...
def "main org roam dailies today create" [] {
  emacsclient -e "(save-window-excursion (org-roam-dailies-capture-today) (org-capture-finalize))"
}
